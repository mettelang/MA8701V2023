...
```{r}
eps=0.01
err=seq(0+eps,1-eps,length=100)
alpha=log((1-err)/err)
mult=exp(alpha)
ds=data.frame(x=err,y=alpha,z=mult)
p1=ggplot(data=ds)+geom_line(aes(x=x,y=y))+xlab("err")+ylab("alpha")
p2=ggplot(data=ds)+geom_line(aes(x=y,y=z))+xlab("alpha")+ylab("exp(alpha)")+geom_hline(yintercept=1)
ggarrange(p1,p2)
```

# Running the 10.2 example
<https://waxworksmath.com/Authors/G_M/Hastie/WWW/chapter_10.html>
