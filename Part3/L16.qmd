...
The use of ensembles like the stacked ensemble (Super Learner) may be seen as an
...
used (MSE, ROC-AUC, misclassification rate, ...).
...
For simplicity assume that $d=1$. The number of positions to consider is
$n$.
...
The random vector $\boldsymbol{Y}_{p\times 1}$ is multivariate normal $N_p$
with mean $\boldsymbol{\mu}$ and (positive definite) covariate matrix
$\Sigma$. The pdf is:
...
2.  Linear combinations of components of $\boldsymbol{Y}$ are (multivariate)
    normal (can be easily proven using moment generating functions MGF).
3.  All subsets of the components of $\boldsymbol{Y}$ are (multivariate)
...
5.  $\boldsymbol{A}\Sigma\boldsymbol{B}^T=\boldsymbol{0} \Leftrightarrow \boldsymbol{A}\boldsymbol{Y}$
