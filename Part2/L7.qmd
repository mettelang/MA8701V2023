...
* [ESL] The Elements of Statistical Learning: Data Mining, Inference, and Prediction, Second Edition (Springer Series in Statistics, 2009) by Trevor Hastie, Robert Tibshirani, and Jerome Friedman. [Ebook](https://web.stanford.edu/~hastie/Papers/ESLII.pdf). Chapter 3.2.2, 3.4.1.

* [HTW] Hastie, Tibshirani, Wainwrigh: "Statistical Learning with Sparsity: The Lasso and Generalizations". CRC press. [Ebook](https://hastie.su.domains/StatLearnSparsity/). Chapter 2.1.
...
Part 2 of the course is called **Shrinkage and regularization for linear and generalized linear modESL**.
...
1. $\text{E}(\boldsymbol{\varepsilon})=\mathbf{0}$.
...
[One version of a solution to 1](./proofbetahatv2.png)
(another version in class notes L2 in 2021)
...
To choose the optimal penalty parameter $\lambda$ cross-validation is the default method in use. ESL recommends to either 
...
In  ESL Ch 7.6 we defined the effective number of parameters (here now referred to as the _effective degrees of freedom_) for a linear smoother $\hat{\mathbf y}={\mathbf Sy}$ as
...
## Missing from presentation (to be added)

In L9 we will "need" to know that the solution to the ridge regression can be found by an extended response vector and covariance matrix followed by ordinary least squares.

Then the response is augmented by a 0-vector of length $p$ and the covariance matrix by a $p \times p$ matrix with $\sqrt{\lambda}$ on the diagonal (else 0)$. 

That should have been included.

Also we did not say that the ridge penalty is the squared $L_2$ loss. Remember that it is squared. 
...
The LS is unbiased with the smallest variance among linear predictors: ESL exercise 3.3a
...
* [Gauss-Markov theorem 3.3a](./ESLe33a.pdf)
...
* [Orthonormal design matrix](./L7ExRidgeOrth.pdf)

```{r}
btb=1
sigma=1
p=6
lam=seq(0,10,length=100)
mse=btb*lam^2/(1+lam)^2+p*(sigma^2)/(1+lam)^2
plot(lam,mse,type="l")
abline(h=p*sigma^2,col=2)
#abline(v=2*sigma^2/btb)
````
