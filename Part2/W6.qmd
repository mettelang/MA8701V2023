...
-   Test data:
    ...
    -   Fit ordinary LS or GLM model with only the non-zero lasso
        covariates
...
-   using the duality of $p$-values and two-sided confidence intervals.
    That is,
-   a $(1-\alpha)$ 100% CI contains values $c$ where the $p$-value is
    below $\alpha$ for testing $H_0: \beta_j=c$. A closed form solution
    involving $P_j$ is found.
...
-   At step $k$ add the predictor that gives the most decrease in the
    sums of squares of error (here now - to follow the notation denoted
    by RSS instead of what we previously called SSE)

$$ \text{RSS}=\sum_{i=1}^N (y_i -\hat{y}_i)^2$$ where $\hat{y}_i$ is the
...
for $p=10$ and $p=50$. The figure was made using Monte Carlo sampling.
...
The $p$-value at step $k$ would be dependent on the number of covariates
$p$.
...
-   [Wessel N. van Wieringen: Lecture notes on ridge
    regression](https://arxiv.org/pdf/1509.09169.pdf)
-   A. Chatterjee and S. N. Lahiri (2011). Bootstrapping Lasso
    Estimators. Journal of the American Statistical Association. Vol.
    106, No. 494 (June 2011), pp. 608-625 (18 pages)
-   Single/multi-sampling splitting part of [Dezeure, Bühlmann, Meier,
    Meinshausen
    (2015)](https://projecteuclid.org/download/pdfview_1/euclid.ss/1449670857).
    "High-Dimensional Inference: Confidence Intervals, p-Values and
...
-   Berk, Richard; Brown, Lawrence; Buja, Andreas; Zhang, Kai; Zhao,
    Linda. Valid post-selection inference. Ann. Statist. 41 (2013), no.
    2, 802--837. doi:10.1214/12-AOS1077.
