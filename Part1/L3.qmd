...
We finished 1) in L2, now we continue!

---

# Expected prediction error - the start
...
as before the expected value is with respect to $(X,Y)$, but the training set is fixed - so that this is the test set error is for this specific training set ${\cal T}$. This is a different way of conditioning than we have looked at before.
...
Why do we want to estimate  $\text{Err}_{\cal T}$ and not $\text{Err}$?
...
In a data poor situation we often want to use the training data to both fit a model but to also do model selection. 

We may do this due to some results we soon will derive (on expected optimism).

For model assessment we do in generel want a separate test set, but also solution using cross-validation or bootstrapping are used.

In addition - given that you are in a data rich, the results we will look into may not be something you will need in your everyday work. However, the results on how to estimate the Err and Err$_{\cal T}$ can give insight into existing methods and situations!


# Expected prediction error - compared to training error

## Training error

(also referred to as apparent error)

For a regression problem the squared loss is popular and the training error is the average loss over the training sample:
...
The training error $\overline{\text{err}}$ is not a good estimate for neighter the $\text{Err}_{\cal T}$ nor the $\text{Err}$.
...
* [7.9](https://github.com/mettelang/MA8701V2023/blob/main/Part1/ELSe79.html)
