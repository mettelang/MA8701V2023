...
[Graphical overview of the topics covered in W3](https://github.com/mettelang/MA8701V2023/blob/main/Part1/W3overview.pdf)
...
Presented in writing in class, see [classnotes lecture 6](https://github.com/mettelang/MA8701V2023/blob/main/Part1/L6beamer20230127.pdf)
...
[Note on these three methods](https://github.com/mettelang/MA8701V2023/blob/main/Part1/Imp3methLM.pdf).

For the Bayesian part, [the Wikipedia entry for Bayesian linear regression](https://en.wikipedia.org/wiki/Bayesian_linear_regression) is useful reading, and we will also look into this in Part 2 on ridge regression (L7).
...
1. Calculate $\dot\phi$ and $\hat\phi$ by Steps 1-8 of 
   Algorithm 3.1.
   
2. Calculate 
   $\dot\eta(i,j)=|X_i^\mathrm{obs}\hat\phi-X_j^\mathrm{mis}\dot\phi|$ 
...
* Model assessement without a test set may be done using an outer bootstrap loop around the MI-loop. Then the .632+ boostrap estimator is preferred in @BootMIWahl2016. For this publication a model selection step was not present - only model assessment. 
