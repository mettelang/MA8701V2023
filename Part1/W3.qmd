...
We will use different letters for response and covariates, but often that is not done in other sources. By response we mean the response in the intended _analysis model_ and ditto for the covariates. (We will later also talk about an _imputation model_ but this is not connected to our notation here.)
...
If no one in the group remembers - maybe look at the Titanic or air quality dataset?
